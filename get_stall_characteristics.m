function [Vstall, a_stall] = get_stall_characteristics(plane, criterion_surface_name, aed_data)
    ...
    obj_function = @(V) interp1(aed_data.speeds, crit_sec_cls, V) - clmax;

    Vstall = fsolve(obj_function, mean(aed_data.speeds));

    a_stall = interp1(aed_data.speeds, cat(1, aed_data.stdata.alpha), Vstall);
